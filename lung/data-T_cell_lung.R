...
# ------------ source -----------------------
# data: Global characterization of T cells in non-small-cell lung cancer by single-cell sequencing
...
# ------------ data processing --------------
data.count <- readRDS("./data/T-cell/lung/lung_natmed18_zemin_raw_count.rds")
...
data.gene <- readRDS("./data/T-cell/lung/lung_natmed18_zemin_gene_info.rds")
data.cell <- readRDS("./data/T-cell/lung/lung_natmed18_zemin_cell_info.rds")
anno.ref <- subset(data.cell, select = c("UniqueCell_ID", "Patient", "majorCluster"))
...
for (i in 1:length(temp1)){
    for (j in 1:length(temp1)){
        if (anno.ref$UniqueCell_ID[j] == temp1[i]){
            temp2[i] <- as.character(anno.ref$majorCluster[j])
            temp3[i] <- as.character(anno.ref$Patient[j])
            break
        }
    }
}
...
saveRDS(raw.data, file="./data/T-cell/lung/data_for_scPred.rds")
saveRDS(cellType, file="./data/T-cell/lung/anno_for_scPred.rds")
# -------------------------------------------

# ------------ dataset ----------------------
data <- readRDS("./data/T-cell/lung/data_for_scPred.rds")
label <- readRDS("./data/T-cell/lung/anno_for_scPred.rds")
write.csv(data, "./data/T-cell/lung/data_for_scPred.csv")
write.csv(label, "./data/T-cell/lung/anno_for_scPred.csv")
set.seed(1234)
train.ratio=0.8
